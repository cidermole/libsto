...
TreeNodeMemory<Token>::TreeNodeMemory(std::string filename, std::shared_ptr<void>, ITreeNode<Token> *parent, Vid vid, size_t maxArraySize):
    TreeNode<Token, SuffixArrayMemory<Token>>(parent, vid, maxArraySize)
{
...
void TreeNodeMemory<Token>::AddLeaf(Vid vid) {
  this->children_[vid] = new TreeNodeMemory<Token>("", nullptr, this, vid, this->kMaxArraySize);
}
