...
TEST(DocumentMapTests, save_load_append) {
  ...
  for(size_t i = 0; i < domain_names.size(); i++) {
    for(size_t j = 0; j < line_counts[i]; j++) {
      docmap_build.AddSentence(nlines, docmap_build.FindOrInsert(domain_names[i], sto_updateid_t{static_cast<stream_t>(-1), nlines + 1}), nlines + 1);
      nlines++;
    }
  }
  ...
  // append
  docmap.AddSentence(nlines, docmap.FindOrInsert("domAdd", sto_updateid_t{static_cast<stream_t>(-1), nlines + 1}), nlines + 1);
  nlines++;
