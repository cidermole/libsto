...
namespace map_helper {
  ...
  inline size_t get(std::unordered_map<size_t, size_t>& m, size_t key) {
    size_t defaultVal = key;
    std::unordered_map<size_t, size_t>::iterator it = m.find(key);
    if(it != m.end())
      return it->second;
    else
      return defaultVal;
...
void random_indices_unsort(size_t k, size_t n, Engine& randomEngine, std::vector<size_t>& out) {
  ...
  for(size_t i = 0; i < k; i++) {
    ...
    std::uniform_int_distribution<size_t> uniform(i, n-1);
